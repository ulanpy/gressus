...
def _emit_json(out_obj: Any) -> None:
    """Stdout is JSONL for downstream tools; exit quietly if the reader closed the pipe."""
    line = json.dumps(out_obj, separators=(",", ":"), ensure_ascii=False)
    try:
...
def main() -> None:
    ...
    sock.bind((host, port))
    sock.listen(1)
    print(f"Listening on {host}:{port} (waiting for wavex-bridge)...", file=sys.stderr, flush=True)
