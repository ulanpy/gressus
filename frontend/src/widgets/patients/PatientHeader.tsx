...
export function PatientHeader({
  ...
  const handleArchive = async () => {
    await workflow.archivePatient(patient.id)
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="min-w-52">
              <DropdownMenuLabel className="text-xs font-semibold text-slate-500">
                {t.workflow.switchPatient}
              </DropdownMenuLabel>
              {workflow.patients.map((p) => (
                <DropdownMenuItem
                  key={p.id}
                  disabled={disabled}
                  className={cn(p.id === patient.id && 'bg-slate-100 font-semibold')}
                  onClick={() => workflow.selectPatient(p.id)}
                >
                  {p.display_name}
                </DropdownMenuItem>
              ))}
