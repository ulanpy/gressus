    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_30px_rgb(15_23_42/0.04)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <ToggleGroup
          ...
          value={metric}
          onValueChange={(next) => {
            if (next) setMetric(next as TelemetryMetric)
          }}
        ...
        >
          {METRICS.map((entry) => (
            <ToggleGroupItem
              key={entry}
              value={entry}
              className="rounded-full border px-3 py-1.5 text-xs font-extrabold shadow-none data-[state=on]:border-slate-950 data-[state=on]:bg-slate-950 data-[state=on]:text-white data-[state=off]:border-slate-200 data-[state=off]:bg-white data-[state=off]:text-slate-700"
            >
              {metricLabel(entry, t)}
            </ToggleGroupItem>
          ))}
