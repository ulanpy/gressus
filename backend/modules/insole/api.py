...
from backend.modules.insole.schemas import InsoleSize, SourceMode, frame_to_dict
...
def frame(
    ...
    return frame_to_dict(service.frame(size=size, source=source, threshold_kpa=threshold_kpa))
...
async def insole_ws(
    ...
    service = InsoleService(
        ...
        started_at=websocket.app.state.started_at,
        config=websocket.app.state.config,
    )
    delay = 1.0 / hz
    try:
        while True:
            frame = service.frame(size=size, source=source, threshold_kpa=threshold_kpa)
